    ...
    public struct CalculateAveragePollutionJob : IJobChunk
    {
        ...
        public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
        {
            ...
            for (int i = 0; i < chunk.Count; i++)
            {
                ...
                int2 airPollutionBonuses = XCellMapSystemRe.GetAirPollutionBonuses(property, ref this.m_TransformFromEntity, this.m_AirPollutionMap, cityModifiers, in this.m_HappinessParameters);
                num3 += airPollutionBonuses.x + airPollutionBonuses.y;
                num6++;
                int2 groundPollutionBonuses = XCellMapSystemRe.GetGroundPollutionBonuses(property, ref this.m_TransformFromEntity, this.m_GroundPollutionMap, cityModifiers, in this.m_HappinessParameters);
                num += groundPollutionBonuses.x + groundPollutionBonuses.y;
                num4++;
                int2 noiseBonuses = XCellMapSystemRe.GetNoiseBonuses(property, ref this.m_TransformFromEntity, this.m_NoisePollutionMap, in this.m_HappinessParameters);
