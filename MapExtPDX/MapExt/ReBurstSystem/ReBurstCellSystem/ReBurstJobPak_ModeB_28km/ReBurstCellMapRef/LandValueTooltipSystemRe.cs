...
namespace MapExtPDX.MapExt.ReBurstSystemModeB
{
    ...
    public struct LandValueTooltipJob : IJob
    {
        ...
        public void Execute()
        {
            ...
            TerrainAttractiveness attractiveness = XCellMapSystemRe.TerrainAttractivenessSystemGetAttractiveness(m_RaycastPosition, m_AttractiveMap);
            ...
            m_GroundPollutionResult.value = XCellMapSystemRe.GroundPollutionSystemGetPollution(m_RaycastPosition, m_GroundPollutionMap).m_Pollution;
            m_AirPollutionResult.value = XCellMapSystemRe.AirPollutionSystemGetPollution(m_RaycastPosition, m_AirPollutionMap).m_Pollution;
            m_NoisePollutionResult.value = XCellMapSystemRe.NoisePollutionSystemGetPollution(m_RaycastPosition, m_NoisePollutionMap).m_Pollution;
        }

        public static int GetCellIndex(float3 pos)
        {

            int num = XCellMapSystemRe.kMapSize / XCellMapSystemRe.LandValueSystemkTextureSize;
            return Mathf.FloorToInt((XCellMapSystemRe.kMapSize / 2 + pos.x) / num) + Mathf.FloorToInt((XCellMapSystemRe.kMapSize / 2 + pos.z) / num) * XCellMapSystemRe.LandValueSystemkTextureSize;
        }
    }
