...
using MapExtPDX.MapExt.Core;
    ...
    /// <summary>
    /// 交通氛围系(Traffic Ambience System)
    /// <para>
    /// 负责管理全图的交通噪氛围数据。定期更新每个网格单元的交通强度供UI显示
    /// </para>
    /// </summary>
    public partial class TrafficAmbienceSystemMod : BaseCellMapSystem, IJobSerializable
    {
        ...
        public static readonly int kTextureSize = XCellMapSystemRe.TrafficAmbienceSystemkTextureSize; // mod尺寸
...
// [ENCODING_FIX]             Mod.Info($"[{typeof(ModSystem).Name}] 创建自定义纹 {typeof(TargetSystem).Name} kTextureSize 原值{TargetSystem.kTextureSize} 变更目标值{this.m_TextureSize.x}");
        ...
        /// <summary>
        /// 负责更新交通氛围数据的并行 Job
        /// <para>核心逻辑是将累积的流量(Accumulator) 应用到当前流量(Traffic) 中，并重置累积器/para>
        /// </summary>
        [BurstCompile]
