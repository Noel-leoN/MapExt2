...
namespace MapExtPDX.MapExt.ReBurstSystemModeC
{
    ...
    /// 影响UI信息视图中的道路显示可适土壤污染影响
    /// 住宅分区时显示居住适应性，出现较频繁，有必要修
    /// </summary>
        ...
        public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
        {
            ...
            else if (chunk.Has(ref this.m_ResourceAvailabilityType) && this.GetResourceAvailabilityData(chunk, out availabilityData, out activeData2))
            {
                ...
                for (int j = 0; j < nativeArray.Length; j++)
                {
                    ...
                    // mod重定
                    GroundPollution pollution = XCellMapSystemRe.GroundPollutionSystemGetPollution(position, this.m_GroundPollutionMap);
                    GroundPollution pollution2 = XCellMapSystemRe.GroundPollutionSystemGetPollution(position2, this.m_GroundPollutionMap);
                    NoisePollution pollution3 = XCellMapSystemRe.NoisePollutionSystemGetPollution(position, this.m_NoisePollutionMap);
                    NoisePollution pollution4 = XCellMapSystemRe.NoisePollutionSystemGetPollution(position2, this.m_NoisePollutionMap);
                    AirPollution pollution5 = XCellMapSystemRe.AirPollutionSystemGetPollution(position, this.m_AirPollutionMap);
                    AirPollution pollution6 = XCellMapSystemRe.AirPollutionSystemGetPollution(position2, this.m_AirPollutionMap);
                    // mod end;
