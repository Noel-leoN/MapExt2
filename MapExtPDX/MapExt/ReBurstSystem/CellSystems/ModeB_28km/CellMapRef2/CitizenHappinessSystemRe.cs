        ...
        public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
        {
            ...
            for (int i = 0; i < chunk.Count; i++)
            {
                ...
                if (this.m_Properties.TryGetComponent(household, out var componentData3) && this.m_Prefabs.TryGetComponent(componentData3.m_Property, out var componentData4))
                {
                    ...
                    // mod;
                    int7 = XCellMapSystemRe.GetGroundPollutionBonuses(property, ref this.m_Transforms, this.m_PollutionMap, cityModifiers, in this.m_CitizenHappinessParameters);
                    value8.x += int7.x + int7.y;
                    value8.z += int7.x;
                    value8.w += int7.y;
                    value8.y++;
                    int8 = XCellMapSystemRe.GetAirPollutionBonuses(property, ref this.m_Transforms, this.m_AirPollutionMap, cityModifiers, in this.m_CitizenHappinessParameters);
                    value3.x += int8.x + int8.y;
                    value3.z += int8.x;
                    value3.w += int8.y;
                    value3.y++;
                    int9 = XCellMapSystemRe.GetNoiseBonuses(property, ref this.m_Transforms, this.m_NoisePollutionMap, in this.m_CitizenHappinessParameters);
