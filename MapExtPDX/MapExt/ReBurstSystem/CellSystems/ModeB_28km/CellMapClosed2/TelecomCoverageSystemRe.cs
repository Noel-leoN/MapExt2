...
namespace MapExtPDX.ModeB
{
    ...
    /// </summary>
    /// 注意该系统没有定义kTextureSize字段，而是直接使用128在job中硬编码
    /// 该系统实用意义不大，建议不作TextureSize修改

    // 重定向私有结构体
    public struct CellDensityData
    {
        public ushort m_Density;
    }

    // 重定向私有结构体
    public struct CellFacilityData
    {
    ...
    public struct TelecomCoverageJob : IJob
    {
        ...
        private void AddNetworkCapacity(NativeArray<CellDensityData> densityData, NativeArray<CellFacilityData> facilityData, NativeList<float> signalStrengths, ArchetypeChunk chunk, ref int arrayIndex, ref TelecomStatus status, DynamicBuffer<CityModifier> cityModifiers)
        {
            ...
            for (int i = 0; i < nativeArray.Length; i++)
            {
                ...
                {
                    //
                    int2 @int = math.max(CellMapSystem<TelecomCoverage>.GetCell(transform.m_Position - componentData.m_Range, XCellMapSystemRe.kMapSize, 128), 0); //

                    int2 int2 = math.min(CellMapSystem<TelecomCoverage>.GetCell(transform.m_Position + componentData.m_Range, XCellMapSystemRe.kMapSize, 128) + 1, 128); //
        ...
        private void CalculateSignalStrength(NativeArray<CellFacilityData> facilityData, NativeArray<float> obstructSlopes, NativeList<float> signalStrengths, ArchetypeChunk chunk, DynamicBuffer<CityModifier> cityModifiers)
        {
            ...
            for (int i = 0; i < nativeArray.Length; i++)
            {
                ...
                int2 @int = math.max(CellMapSystem<TelecomCoverage>.GetCell(position - componentData.m_Range, XCellMapSystemRe.kMapSize, 128), 0); //
                int2 int2 = math.min(CellMapSystem<TelecomCoverage>.GetCell(position + componentData.m_Range, XCellMapSystemRe.kMapSize, 128) + 1, 128); //
                int2 int3 = int2 - @int;
                if (math.any(int3 <= 0))
                {
                    continue;
                ...
                else
                {
                    ...
                    int2 int4 = math.clamp(CellMapSystem<TelecomCoverage>.GetCell(position, XCellMapSystemRe.kMapSize, 128), 0, 127); //
        ...
        private void CalculateSignalStrength(NativeArray<float> signalStrengthArray, int2 min, int2 max, float range, float3 position)
        {
            ...
            for (int i = min.y; i < max.y; i++)
            {
                for (int j = min.x; j < max.x; j++)
                {
                    float3 cellCenter = XCellMapSystemRe.GetCellCenter(new int2(j, i), 128); //
        ...
        private void CalculateCellSignalStrength(NativeArray<float> obstructSlopes, NativeArray<float> signalStrengthArray, int2 cell, int2 min, int2 max, float range, float3 position)
        {
            ...
            float3 cellCenter = XCellMapSystemRe.GetCellCenter(cell, 128); //
            ...
            float num4 = (float)XCellMapSystemRe.kMapSize / 128f; //
        ...
        {
            int2 @int = math.clamp(CellMapSystem<TelecomCoverage>.GetCell(position, XCellMapSystemRe.kMapSize, 128), 0, 127); //
