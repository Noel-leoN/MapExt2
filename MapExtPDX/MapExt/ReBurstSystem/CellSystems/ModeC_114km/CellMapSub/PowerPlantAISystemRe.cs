    ...
    public struct PowerPlantTickJob : IJobChunk
    {
        ...
        public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
        {
            ...
            for (int i = 0; i < chunk.Count; i++)
            {
                ...
                float2 float2 = Unity.Mathematics.float2.zero;
                if (componentData5.m_Production > 0)
                {
                    // mod
                    Wind wind = XCellMapSystemRe.WindSystemGetWind(nativeArray6[i].m_Position, this.m_WindMap);
                ...
                float2 zero2 = Unity.Mathematics.float2.zero;
                ...
                float2 float3 = Unity.Mathematics.float2.zero;
        ...
        {
            // mod
            float num = (float)XCellMapSystemRe.GroundWaterSystemGetGroundWater(position, groundWaterMap).m_Amount / (float)groundWaterData.m_MaximumGroundWater;
