...
namespace MapExtPDX.ModeC
{
        ...
        public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
        {
            ...
            for (int i = 0; i < chunk.Count; i++)
            {
                ...
                if (componentData.m_Types != AllowedWaterTypes.None)
                {
                    ...
                    {
                        // mod XCellMapSystemRe.GroundWaterSystemGetGroundWater
                        int num2 = Mathf.RoundToInt(math.clamp((float)XCellMapSystemRe.GroundWaterSystemGetGroundWater(nativeArray3[i].m_Position, this.m_GroundWaterMap).m_Max / this.m_Parameters.m_GroundwaterPumpEffectiveAmount, 0f, 1f) * (float)componentData.m_Capacity);
                        num += num2;
                    }
    ...
    public struct GroundWaterPumpJob : IJobChunk
    {
        ...
        public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
        {
            ...
            for (int i = 0; i < chunk.Count; i++)
            {
                ...
                {
                    // mod XCellMapSystemRe.GroundWaterSystemGetGroundWater 
                    int num = Mathf.CeilToInt(math.clamp((float)XCellMapSystemRe.GroundWaterSystemGetGroundWater(nativeArray3[i].m_Position, this.m_GroundWaterMap).m_Max / this.m_Parameters.m_GroundwaterPumpEffectiveAmount, 0f, 1f) * (float)componentData.m_Capacity);
    ...
    public struct GroundWaterReservoirJob : IJob
    {
        ...
        public void Execute()
        {
            ...
            foreach (int2 item3 in this.m_TempGroundWaterPumpCells)
            {
                this.EnqueueIfUnprocessed(item3, processedCells);
            }
            int2 item;
            while (this.m_Queue.TryDequeue(out item))
            {
                int index = item.x + item.y * XCellMapSystemRe.GroundWaterSystemkTextureSize;
        ...
        {
            if (XCellMapSystemRe.GroundWaterSystemIsValidCell(cell) && processedCells.Add(cell))
            {
