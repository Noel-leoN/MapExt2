...
namespace MapExtPDX.ModeA
{
    ...
    public struct PolluteGroundWaterJob : IJob
    {
        ...
        public void Execute()
        {
            int kTextureSize = XCellMapSystemRe.GroundWaterSystemkTextureSize;

            for (int i = 0; i < m_GroundWaterMap.Length; i++)
            {
                GroundWater value = m_GroundWaterMap[i];
                GroundPollution pollution = XCellMapSystemRe.GroundPollutionSystemGetPollution(XCellMapSystemRe.GetCellCenter(i, kTextureSize), m_PollutionMap);
