            ...
            Dictionary<string, int> indexCounts)
        {
            var entries = new Dictionary<string, string>
            {
                ...
                },
                {
                    ...
                    " - Off: 原版行为，始终模拟。\n" +
                    " - Auto: 仅在明确无雪的天气下冻结（气温明显高于冰点且无降水），一旦转冷或开始降水立即自动解冻，无可见副作用。\n" +
                    " - Always: 最省，但降雪不再积累、积雪不再消融。\n\n" +
                    "★ 建议：Auto — 在不改变积雪表现的前提下，省掉夏季空转的全图计算。\n" +
                    "★ 提示：该选项即时生效无须重启。Dev Debug 面板的 snow sim speed 仍然有效：" +
                    "在那里设定的数值会在解冻时恢复。"
                },
                ...
                { m_Setting.GetOptionLabelLocaleID(nameof(ModSettings.EnableVanillaConversion)), "⚠ 原版地图扩展 (高度实验性)" },
                {
                    ...
                    " - 扩展区域可能出现模拟不稳定\n\n" +
                    "★ 使用风险自负。与「禁用背景世界地图」互斥——启用其一，另一项会变灰不可选。\n" +
                    "⚠ 启用后需重启游戏。"
                },
                ...
                { m_Setting.GetOptionLabelLocaleID(nameof(ModSettings.TerrainBrushRoadBlock)), "地形笔刷道路阻挡 (测试)" },
                {
                    ...
                    "道路自动成为地形编辑的边界，方便河岸修整。\n\n" +
                    "★ 提示：即时生效，无需重启。"
                },
                { m_Setting.GetOptionLabelLocaleID(nameof(ModSettings.TerrainBrushRoadMargin)), "道路阻挡宽度余量 (m)" },
                {
                    m_Setting.GetOptionDescLocaleID(nameof(ModSettings.TerrainBrushRoadMargin)),
                    "道路边缘两侧的额外保护宽度。值越大，道路附近受保护的区域越宽。" +
                    "改变后下一笔立刻生效。\n\n" +
                    "★ 建议值：大多数道路 1.0 ~ 4.0m。"
                },
                ...
                { m_Setting.GetOptionLabelLocaleID(nameof(ModSettings.CellMapProfiling)), "启用 CellMap 诊断计时（地下水）" },
                {
                    m_Setting.GetOptionDescLocaleID(nameof(ModSettings.CellMapProfiling)),
                    "仅供诊断。开启后地下水的 tick job 改为在主线程同步执行，并对三趟各自计时，把毫秒数写进 MapExtPDX.log。\n\n" +
                    "★ 注意：这会让主线程每个 tick 被阻塞一次（每 128 模拟帧，约 2 秒），游戏会有可感知的周期性卡顿。量到数字就关掉。\n" +
                    "★ Burst 仍然生效，所以三趟之间的相对比例可靠，只是绝对值会略高于实际在 worker 上的耗时。"
                },
                ...
                { m_Setting.GetOptionLabelLocaleID(nameof(ModSettings.HeightmapExportDirection)), "高程图导出方向" },
                {
                    m_Setting.GetOptionDescLocaleID(nameof(ModSettings.HeightmapExportDirection)),
                    "导出 PNG 时应用的行序变换。游戏内部高程图的行序与 PNG 文件格式的约定相反，故正确方向取决于你的用途。\n★ 不确定时选「All」一次输出四份变体再比对。"
                },
                { m_Setting.GetOptionLabelLocaleID(nameof(ModSettings.HeightmapExportRaw)), "同时导出 RAW" },
                {
                    m_Setting.GetOptionDescLocaleID(nameof(ModSettings.HeightmapExportRaw)),
                    "额外输出一份无文件头的 16-bit RAW，供外部工具程序化处理。\n★ 地图编辑器不接受 .raw 格式，只有 PNG 能导入回游戏。"
                },
                { m_Setting.GetOptionLabelLocaleID(nameof(ModSettings.HeightmapExportReport)), "高程图导出结果" },
                {
                    m_Setting.GetOptionDescLocaleID(nameof(ModSettings.HeightmapExportReport)),
                    "显示最近一次导出的结果：文件数量、分辨率与目标文件夹。"
                },
                { m_Setting.GetOptionLabelLocaleID(nameof(ModSettings.ExportHeightmap)), "导出高程图" },
                {
                    m_Setting.GetOptionDescLocaleID(nameof(ModSettings.ExportHeightmap)),
                    "将当前地形高程图导出为 16-bit PNG 至 Heightmaps 文件夹，可直接在地图编辑器的「导入高程图」面板中选用。\n★ 仅导出纯地形，不含建筑、道路与地块压平，重新导入不会把这些痕迹固化进地形。\n★ 需先加载地图或存档。"
                },
            };
            return entries;
        }
