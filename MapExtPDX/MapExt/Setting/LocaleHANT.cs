    ...
    public class LocaleHANT : IDictionarySource
    {
            ...
            Dictionary<string, int> indexCounts)
        {
            var entries = new Dictionary<string, string>
            {
                ...
                { m_Setting.GetOptionLabelLocaleID(nameof(ModSettings.TerrainDownsampleThrottle)), "背景地形降採樣事件化" },
                {
                    m_Setting.GetOptionDescLocaleID(nameof(ModSettings.TerrainDownsampleThrottle)),
                    "當地形級聯未被重繪時，跳過每幀的背景高度圖降採樣 dispatch。\n\n" +
                    "原版每幀無條件執行這次 compute dispatch，但其輸出只在級聯實際重繪時才會改變——" +
                    "相機靜止且無地形修改時，逐幀結果完全相同。\n\n" +
                    "僅對有背景世界地圖的存檔有效；若已停用背景世界地圖，本選項無作用。\n\n" +
                    "★ 建議：存檔有背景世界地圖時開啟，GPU 收益小但穩定。\n" +
                    "★ 提示：該選項即時生效無須重啟。"
                },
                {
                ...
                },
                {
                    ...
                    " - ⚠ Adaptive: 自適應休眠（實驗性）：水面收斂靜止後凍結模擬，偵測到擾動（地形修改、水源變更、海平面調整、海嘯／洪水事件、降雨）時立即喚醒。與 Minimal 的固定降頻不同，本檔位是內容自適應——靜止時近乎零開銷，流動時全保真。另有週期性心跳重新評估收斂狀態，故水面不會永久凍結。編輯器中停用。\n" +
                    " - Paused: 暫停流體：完全凍結水體流動計算（水面將靜止但水位不會出現大變化）。\n\n" +
                    "★ 提示：該選項即時生效無須重啟。"
                },
                {
                    m_Setting.GetOptionLabelLocaleID(nameof(ModSettings.WaterAsyncCompute)),
                    "⚠ 水體 Async Compute（實驗性）"
                },
                {
                    m_Setting.GetOptionDescLocaleID(nameof(ModSettings.WaterAsyncCompute)),
                    "讓水模擬走獨立的 GPU Compute 佇列，與圖形渲染管線並行執行，主要改善「顯示卡吃緊（GPU-bound）」場景的幀時間。\n\n" +
                    "⚠ 注意：此為原版預設關閉的實驗性功能。\n" +
                    " - 主要改善 GPU 幀時間，對「CPU 吃緊」（晚期大城市、尋路瓶頸）幾乎無效。\n" +
                    " - 收益與風險高度依賴顯示卡與驅動：高階獨顯通常有感，整合顯示晶片／老顯卡／筆電可能無感甚至倒退。\n" +
                    " - 極少數驅動可能出現水面閃爍或延遲一幀，若發現異常請關閉。\n\n" +
                    "★ 建議：預設關閉；獨顯玩家可自行實測開關前後的幀時間與畫面正確性。\n" +
                    "★ 提示：該選項即時生效無須重啟。"
                },
                {
                ...
                },
                {
                    ...
                    "遊戲暫停時，原版仍會每幀空轉整條水模擬 GPU 管線（時間步長為 0，物理不推進，純屬浪費）。\n\n" +
                    "開啟後暫停即凍結水模擬，取消暫停自動恢復；放置建築等地形變更期間會自動讓行，待水面適應新地形後再凍結。\n\n" +
                    "★ 建議：保持開啟。近零風險——水面流動動畫由著色器時間驅動，不受影響。\n" +
                    "★ 提示：該選項即時生效無須重啟。"
                },
                {
                ...
                { m_Setting.GetOptionLabelLocaleID(nameof(ModSettings.DetectedConflictMods)), "偵測到的衝突Mod" },
                {
                    ...
                    "列出啟動時偵測到的已知第三方衝突Mod。啟動時一次性掃描，零執行時開銷。"
                },
                ...
                { m_Setting.GetOptionLabelLocaleID(nameof(ModSettings.FindJobMaxCost)), "找工作最高尋路成本" },
                {
                    ...
                    "控制市民為了尋求工作崗位時的最大搜尋範圍上限。該值越高，大地圖遠郊孤島小鎮越容易招到工人。\n" +
                    "★ 建議值：\n" +
                    " - 全地圖通用：200000（預設）"
                },
                ...
                { m_Setting.GetOptionLabelLocaleID(nameof(ModSettings.EnableVehicleRescue)), "• 啟用購車停放救援" },
                {
                    ...
                    "啟用購車救援系統：偵測市民私家車因停車位不足導致遺失（m_Lane == Null）的情況，將其傳送到車主住宅附近重新停放。\n\n" +
                    "★ 同時會自動清理存檔中已存在的存量幽靈車輛：有住宅者救援，無家可歸的孤兒車安全刪除（對舊存檔特別有用）。\n" +
                    "★ 預設關閉。可先用下方「掃描幽靈車輛」按鈕確認存檔中的規模。\n" +
