...
namespace MapExtPDX
{
    ...
    public class ModSettings : ModSetting
    {
        ...
        /// 0% = 环境因子不影响地价（接近原版行为），100% = 环境因子完全传递。
        ...
        /// <summary>
        /// 服务加成上限乘数（百分比）。
        /// 控制服务覆盖（医疗、警务、教育、交通）对 Edge 地价加成的上限缩放。
        /// 100% = Mod 默认上限，50% = 上限减半（降低服务密集区的地价溢价）。
        /// </summary>
        [SettingsUISection(kRentControlTab, kLandValueFactorGroup)]
        ...
        public bool ScanGhostVehicles
        {
            ...
            {
                var system = Unity.Entities.World.DefaultGameObjectInjectionWorld
                    ?.GetExistingSystemManaged<MapExtPDX.EcoShared.P3_VehiclePurchaseRescueSystem>();
        ...
        private string GetPatchModeDisplayName(PatchModeSetting mode)
        {
            switch (mode)
            {
                ...
                case PatchModeSetting.ModeD: return "• ModeD 43km (3x3) (Experimental)";
