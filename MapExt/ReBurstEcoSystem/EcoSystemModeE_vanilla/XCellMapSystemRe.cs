...
namespace MapExtPDX.MapExt.ReBurstEcoSystemModeE
{
    ...
    public static class CellMapSystemRe
    {
        ...
        public const int kMapSize = 14336; //MapSizeMultiplier * 14336;
        ...
        public static float3 AirPollutionSystemGetCellCenter(int index) => GetCellCenter(index, AirPollutionSystemkTextureSize);
        ...
        public static AirPollution AirPollutionSystemGetPollution(float3 position, NativeArray<AirPollution> pollutionMap)
        {
            ...
            int2 cell = GetCell(position - new float3(num / 2f, 0f, num / 2f), kMapSize, AirPollutionSystemkTextureSize);
            float2 @float = GetCellCoords(position, kMapSize, AirPollutionSystemkTextureSize) - new float2(0.5f, 0.5f);
        ...
        public static AvailabilityInfoCell AvailabilityInfoToGridSystemGetAvailabilityInfo(float3 position, NativeArray<AvailabilityInfoCell> AvailabilityInfoMap)
        {
            ...
            if (cell.x < 0 || cell.x >= AvailabilityInfoToGridSystemkTextureSize || cell.y < 0 || cell.y >= AvailabilityInfoToGridSystemkTextureSize)
            {
                return default;
            }

            float4 availabilityInfo = AvailabilityInfoMap[cell.x + AvailabilityInfoToGridSystemkTextureSize * cell.y].m_AvailabilityInfo;
            float4 y = cell.x < AvailabilityInfoToGridSystemkTextureSize - 1 ? AvailabilityInfoMap[cell.x + 1 + AvailabilityInfoToGridSystemkTextureSize * cell.y].m_AvailabilityInfo : (float4)0;
            float4 x = cell.y < AvailabilityInfoToGridSystemkTextureSize - 1 ? AvailabilityInfoMap[cell.x + AvailabilityInfoToGridSystemkTextureSize * (cell.y + 1)].m_AvailabilityInfo : (float4)0;
            float4 y2 = cell.x < AvailabilityInfoToGridSystemkTextureSize - 1 && cell.y < AvailabilityInfoToGridSystemkTextureSize - 1 ? AvailabilityInfoMap[cell.x + 1 + AvailabilityInfoToGridSystemkTextureSize * (cell.y + 1)].m_AvailabilityInfo : (float4)0;
        ...
        public static GroundPollution GroundPollutionSystemGetPollution(float3 position, NativeArray<GroundPollution> pollutionMap)
        {
            ...
            if (cell.x < 0 || cell.x >= GroundPollutionSystemkTextureSize || cell.y < 0 || cell.y >= GroundPollutionSystemkTextureSize)
            {
                return result;
            }

            GroundPollution groundPollution = pollutionMap[cell.x + GroundPollutionSystemkTextureSize * cell.y];
            GroundPollution groundPollution2 = cell.x < GroundPollutionSystemkTextureSize - 1 ? pollutionMap[cell.x + 1 + GroundPollutionSystemkTextureSize * cell.y] : default;
            GroundPollution groundPollution3 = cell.y < GroundPollutionSystemkTextureSize - 1 ? pollutionMap[cell.x + GroundPollutionSystemkTextureSize * (cell.y + 1)] : default;
            GroundPollution groundPollution4 = cell.x < GroundPollutionSystemkTextureSize - 1 && cell.y < GroundPollutionSystemkTextureSize - 1 ? pollutionMap[cell.x + 1 + GroundPollutionSystemkTextureSize * (cell.y + 1)] : default;
            result.m_Pollution = (short)math.round(math.lerp(math.lerp(groundPollution.m_Pollution, groundPollution2.m_Pollution, cellCoords.x - cell.x), math.lerp(groundPollution3.m_Pollution, groundPollution4.m_Pollution, cellCoords.x - cell.x), cellCoords.y - cell.y));
            return result;
        }
        ...
        public static NoisePollution NoisePollutionSystemGetPollution(float3 position, NativeArray<NoisePollution> pollutionMap)
        {
            ...
            int2 cell = GetCell(position - new float3(num / 2f, 0f, num / 2f), kMapSize, NoisePollutionSystemkTextureSize);
            float2 @float = GetCellCoords(position, kMapSize, NoisePollutionSystemkTextureSize) - new float2(0.5f, 0.5f);
            ...
            short pollution4 = pollutionMap[cell.x + 1 + NoisePollutionSystemkTextureSize * (cell.y + 1)].m_Pollution;
            result.m_Pollution = (short)math.round(math.lerp(math.lerp(pollution, pollution2, @float.x - cell.x), math.lerp(pollution3, pollution4, @float.x - cell.x), @float.y - cell.y));
            return result;
        }
        ...
        public static float ZoneAmbienceSystemGetZoneAmbience(GroupAmbienceType type, float3 position, NativeArray<ZoneAmbienceCell> zoneAmbienceMap, float maxPerCell)
        {
            ...
            for (int i = cell.x - 2; i <= cell.x + 2; i++)
            {
                ...
                {
                    if (i >= 0 && i < ZoneAmbienceSystemkTextureSize && j >= 0 && j < ZoneAmbienceSystemkTextureSize)
                    {
                        int index = i + ZoneAmbienceSystemkTextureSize * j;
                        float num3 = math.max(1f, math.distancesq(GetCellCenter(index, ZoneAmbienceSystemkTextureSize), position) / 10f);
        ...
        public static TerrainAttractiveness TerrainAttractivenessSystemGetAttractiveness(float3 position, NativeArray<TerrainAttractiveness> attractivenessMap)
        {
            ...
            if (cell.x < 0 || cell.x >= TerrainAttractivenessSystemkTextureSize || cell.y < 0 || cell.y >= TerrainAttractivenessSystemkTextureSize)
            {
                return result;
            }

            TerrainAttractiveness terrainAttractiveness = attractivenessMap[cell.x + TerrainAttractivenessSystemkTextureSize * cell.y];
            TerrainAttractiveness terrainAttractiveness2 = cell.x < TerrainAttractivenessSystemkTextureSize - 1 ? attractivenessMap[cell.x + 1 + TerrainAttractivenessSystemkTextureSize * cell.y] : default;
            TerrainAttractiveness terrainAttractiveness3 = cell.y < TerrainAttractivenessSystemkTextureSize - 1 ? attractivenessMap[cell.x + TerrainAttractivenessSystemkTextureSize * (cell.y + 1)] : default;
            TerrainAttractiveness terrainAttractiveness4 = cell.x < TerrainAttractivenessSystemkTextureSize - 1 && cell.y < TerrainAttractivenessSystemkTextureSize - 1 ? attractivenessMap[cell.x + 1 + TerrainAttractivenessSystemkTextureSize * (cell.y + 1)] : default;
            result.m_ForestBonus = (short)math.round(math.lerp(math.lerp(terrainAttractiveness.m_ForestBonus, terrainAttractiveness2.m_ForestBonus, cellCoords.x - cell.x), math.lerp(terrainAttractiveness3.m_ForestBonus, terrainAttractiveness4.m_ForestBonus, cellCoords.x - cell.x), cellCoords.y - cell.y));
            result.m_ShoreBonus = (short)math.round(math.lerp(math.lerp(terrainAttractiveness.m_ShoreBonus, terrainAttractiveness2.m_ShoreBonus, cellCoords.x - cell.x), math.lerp(terrainAttractiveness3.m_ShoreBonus, terrainAttractiveness4.m_ShoreBonus, cellCoords.x - cell.x), cellCoords.y - cell.y));
            return result;
        }
        ...
        public static TrafficAmbienceCell TrafficAmbienceSystemGetTrafficAmbience2(float3 position, NativeArray<TrafficAmbienceCell> trafficAmbienceMap, float maxPerCell)
        {
            ...
            for (int i = cell.x - 2; i <= cell.x + 2; i++)
            {
                for (int j = cell.y - 2; j <= cell.y + 2; j++)
                {
                    if (i >= 0 && i < TrafficAmbienceSystemkTextureSize && j >= 0 && j < TrafficAmbienceSystemkTextureSize)
                    {
                        int index = i + TrafficAmbienceSystemkTextureSize * j;
                        float num3 = math.max(1f, math.distancesq(GetCellCenter(index, TrafficAmbienceSystemkTextureSize), position));
        ...
        public static float ZoneAmbienceSystemGetZoneAmbienceNear(GroupAmbienceType type, float3 position, NativeArray<ZoneAmbienceCell> zoneAmbienceMap, float nearWeight, float maxPerCell)
        {
            ...
            for (int i = cell.x - 2; i <= cell.x + 2; i++)
            {
                for (int j = cell.y - 2; j <= cell.y + 2; j++)
                {
                    ...
                    {
                        int index = i + ZoneAmbienceSystemkTextureSize * j;
                        float num3 = math.max(1f, math.pow(math.distance(GetCellCenter(index, ZoneAmbienceSystemkTextureSize), position) / 10f, 1f + nearWeight));
                        num += math.min(maxPerCell, zoneAmbienceMap[index].m_Value.GetAmbience(type)) / num3;
                        num2 += 1f / num3;
                    }
                }
        ...
        public static void GroundWaterSystemConsumeGroundWater(float3 position, NativeArray<GroundWater> groundWaterMap, int amount)
        {
            ...
            {
                Debug.LogWarning($"Trying to consume more groundwater than available! amount: {amount}, available: {totalAvailable}");
            }
        ...
        public static Wind WindSystemGetWind(float3 position, NativeArray<Wind> windMap)
        {
            ...
            Wind result = default;
            result.m_Wind = math.lerp(math.lerp(windMap[cell.x + WindSystemkTextureSize * cell.y].m_Wind, windMap[num + WindSystemkTextureSize * cell.y].m_Wind, cellCoords.x - cell.x), math.lerp(windMap[cell.x + WindSystemkTextureSize * num2].m_Wind, windMap[num + WindSystemkTextureSize * num2].m_Wind, cellCoords.x - cell.x), cellCoords.y - cell.y);
            return result;
        }

        public static WindSimulationSystem.WindCell WindSimulationSystemGetCell(int3 position, NativeArray<WindSimulationSystem.WindCell> cells)
        {
            int3 WindSimulationSystemkResolution = new(WindSystemkTextureSize, WindSystemkTextureSize, 16);
            int num = position.x + position.y * WindSimulationSystemkResolution.x + position.z * WindSimulationSystemkResolution.x * WindSimulationSystemkResolution.y;
        ...
        public static float3 WindSimulationSystemGetCenterVelocity(int3 cell, NativeArray<WindSimulationSystem.WindCell> cells)
        {
            float3 velocities = WindSimulationSystemGetCell(cell, cells).m_Velocities;
            float3 @float = ((cell.x > 0) ? WindSimulationSystemGetCell(cell + new int3(-1, 0, 0), cells).m_Velocities : velocities);
            float3 float2 = ((cell.y > 0) ? WindSimulationSystemGetCell(cell + new int3(0, -1, 0), cells).m_Velocities : velocities);
            float3 float3 = ((cell.z > 0) ? WindSimulationSystemGetCell(cell + new int3(0, 0, -1), cells).m_Velocities : velocities);
        ...
        public static float3 WindSimulationSystemGetCellCenter(int index)
        {
            ...
            int3 @int = new(index % WindSimulationSystemkResolution.x, index / WindSimulationSystemkResolution.x % WindSimulationSystemkResolution.y, index / (WindSimulationSystemkResolution.x * WindSimulationSystemkResolution.y));
            float3 result = kMapSize * new float3((@int.x + 0.5f) / WindSimulationSystemkResolution.x, 0f, (@int.y + 0.5f) / WindSimulationSystemkResolution.y) - kMapSize / 2;
        ...
        public static float GetCachedApartmentQuality(int familySize, int children, int averageHappiness, HouseholdFindPropertySystemMod.GenericApartmentQuality quality)
        {
        ...
        public static HouseholdFindPropertySystemMod.GenericApartmentQuality GetGenericApartmentQuality(Entity building, Entity buildingPrefab, ref Building buildingData, ref ComponentLookup<BuildingPropertyData> buildingProperties, ref ComponentLookup<BuildingData> buildingDatas, ref ComponentLookup<SpawnableBuildingData> spawnableDatas, ref ComponentLookup<CrimeProducer> crimes, ref BufferLookup<Game.Net.ServiceCoverage> serviceCoverages, ref ComponentLookup<Locked> locked, ref ComponentLookup<ElectricityConsumer> electricityConsumers, ref ComponentLookup<WaterConsumer> waterConsumers, ref ComponentLookup<GarbageProducer> garbageProducers, ref ComponentLookup<MailProducer> mailProducers, ref ComponentLookup<Game.Objects.Transform> transforms, ref ComponentLookup<Abandoned> abandoneds, NativeArray<GroundPollution> pollutionMap, NativeArray<AirPollution> airPollutionMap, NativeArray<NoisePollution> noiseMap, CellMapData<TelecomCoverage> telecomCoverages, DynamicBuffer<CityModifier> cityModifiers, Entity healthcareService, Entity entertainmentService, Entity educationService, Entity telecomService, Entity garbageService, Entity policeService, CitizenHappinessParameterData happinessParameterData, GarbageParameterData garbageParameterData)
        {
            HouseholdFindPropertySystemMod.GenericApartmentQuality result = default;
            bool flag = true;
            ...
            // mod;
            healthcareBonuses = GetGroundPollutionBonuses(building, ref transforms, pollutionMap, cityModifiers, in happinessParameterData);
            @int += healthcareBonuses;
            healthcareBonuses = GetAirPollutionBonuses(building, ref transforms, airPollutionMap, cityModifiers, in happinessParameterData);
            @int += healthcareBonuses;
            healthcareBonuses = GetNoiseBonuses(building, ref transforms, noiseMap, in happinessParameterData);
            @int += healthcareBonuses;
            // mod end;
        ...
        {
            HouseholdFindPropertySystemMod.GenericApartmentQuality genericApartmentQuality = GetGenericApartmentQuality(building, buildingPrefab, ref buildingData, ref buildingProperties, ref buildingDatas, ref spawnableDatas, ref crimes, ref serviceCoverages, ref locked, ref electricityConsumers, ref waterConsumers, ref garbageProducers, ref mailProducers, ref transforms, ref abandoneds, pollutionMap, airPollutionMap, noiseMap, telecomCoverages, cityModifiers, healthcareService, entertainmentService, educationService, telecomService, garbageService, policeService, happinessParameterData, garbageParameterData);
