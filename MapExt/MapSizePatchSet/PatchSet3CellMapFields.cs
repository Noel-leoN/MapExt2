...
namespace MapExtPDX.MapExt.MapSizePatchSet
{
    public static class CellMapSystemPatchManager
    {
        ...
        // 日志输出：方法名 -> 修改记录列表
        private static readonly ConcurrentDictionary<string, List<string>> _patchLog = new ConcurrentDictionary<string, List<string>>();

        // 核心映射表: FieldInfo -> int (目标值)
        private static readonly Dictionary<FieldInfo, int> _replacementMap = new Dictionary<FieldInfo, int>(new FieldInfoEqualityComparer());
        ...
        public static void ApplyPatches(Harmony harmony)
        {
            ...
            //int texMult = PatchManager.CurrentCoreValueTex;
            int texMult = mapMult;
        ...
        public static void Register<TDerived>(int mapMultiplier, int textureMultiplier)
        {
            ...
            while (baseType != null)
            {
                ...
                {
                    FieldInfo mapField = baseType.GetField("kMapSize", BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic);
            ...
            FieldInfo texField = derivedType.GetField("kTextureSize", BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic);
        ...
        private static void ScanBaseMethods(Type closedBaseType)
        {
            ...
            var methods = closedBaseType.GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Static | BindingFlags.DeclaredOnly);
            ...
            try
            {
                ...
                if (rawValue is int originalValue)
                {
                    ...
                    if (!_replacementMap.ContainsKey(field))
                    {
                        ...
                        // 日志
                        Info($" Registered {field.DeclaringType.Name}.{field.Name}: {originalValue} -> {newValue} (x{multiplier})");
                    }
                }
            }
            catch (Exception ex)
            {
                Info($" Failed to read initial value for {field.DeclaringType.Name}.{field.Name}. Skipped. Error: {ex.Message}");
            }
        }
        ...
        public static void ApplyAllPatches(Harmony harmony)
        {
            ...
            {
                var members = type.GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Static | BindingFlags.DeclaredOnly)
                    .Cast<MethodBase>()
                ...
                foreach (var method in members)
                {
                    ...
                    if (method.IsAbstract || method.ContainsGenericParameters || method.MethodImplementationFlags.HasFlag(MethodImplAttributes.InternalCall))
                        continue;
            ...
            // 5. 输出详细日志
            PrintSummary(totalTypesScanned, totalMethodsScanned, distinctMethodsToPatch.Count, scanTime, sw.ElapsedMilliseconds);
        }
        ...
        private static bool IsClientMethod(MethodBase method)
        {
            try
            {
                ...
                for (int i = 0; i < il.Length; i++) if (il[i] == 0x7E) { hasLdsfld = true; break; }
                if (!hasLdsfld) return false;

                // 2. 精确筛选：检查 Operand
                foreach (var instr in PatchProcessor.GetOriginalInstructions(method))
                {
                    if (instr.opcode == OpCodes.Ldsfld && instr.operand is FieldInfo field && _replacementMap.ContainsKey(field))
                        return true;
                }
            }
            catch { return false; }
        ...
        private class MethodComparer : IEqualityComparer<MethodBase>
        {
            ...
            public int GetHashCode(MethodBase obj) => obj.MetadataToken.GetHashCode() ^ (obj.DeclaringType?.GetHashCode() ?? 0);
        }
    }
