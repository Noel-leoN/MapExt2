        ...
        public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
        {
            ...
            else if (chunk.Has(ref this.m_ResourceAvailabilityType) && this.GetResourceAvailabilityData(chunk, out availabilityData, out activeData2))
            {
                ...
                for (int j = 0; j < nativeArray.Length; j++)
                {
                    ...
                    // mod重定向
                    GroundPollution pollution = CellMapSystemRe.GroundPollutionSystemGetPollution(position, this.m_GroundPollutionMap);
                    GroundPollution pollution2 = CellMapSystemRe.GroundPollutionSystemGetPollution(position2, this.m_GroundPollutionMap);
                    NoisePollution pollution3 = CellMapSystemRe.NoisePollutionSystemGetPollution(position, this.m_NoisePollutionMap);
                    NoisePollution pollution4 = CellMapSystemRe.NoisePollutionSystemGetPollution(position2, this.m_NoisePollutionMap);
                    AirPollution pollution5 = CellMapSystemRe.AirPollutionSystemGetPollution(position, this.m_AirPollutionMap);
                    AirPollution pollution6 = CellMapSystemRe.AirPollutionSystemGetPollution(position2, this.m_AirPollutionMap);
                    // mod end;
