...
namespace MapExtPDX.MapExt.ReBurstSystemModeC
{
    ...
    // 2. Edge地价计算医疗/教育/警务/商业/公交/电车地铁等服务覆盖加成。CellMap热力图加权采样Edge地价生成，叠加景观吸引力/电信覆盖/地形吸引力，扣除空气/土壤/噪音污染。经济模拟(租金计算)使用的Edge地价与UI显示的热力图地价不一致。
    // 3. 重构目标：Edge地价直接计算最终值，包含所有加成和扣除因素。CellMap热力图加权采样Edge地价生成，确保UI显示与经济模拟一致。
    // 注意：早期版本根据道路边建筑属性计算地价方式对性能影响大，参照中日韩等地基准地价体系/地价公告现值/路线价，不再区分建筑用途对地价的影响。建筑地价影响由RentSystem体现。
    ...
    public partial class LandValueSystemMod : BaseCellMapSystem, IJobSerializable
    {
        ...
        // --- 蔓延与辐射 (热力图生成) ---
        // 地价从道路向周围辐射的物理距离（米）。
        // 原版逻辑依赖格大小(14336/128=112m)，这里改为固定物理距离：1.减少断层；2.可配置辐射范围。
        // 建议值: 150m - 300m (一个街区约 80-120m)
        public const float kSpreadDistance = 150.0f;

        // 数值平滑系数 (Lerp)。越小变化越慢（越平滑），越大变化越快（越灵敏）。
        ...
        public static readonly int kTextureSize = CellMapSystemRe.LandValueSystemkTextureSize; // mod尺寸
        public int2 TextureSize => new int2(kTextureSize, kTextureSize);

        // 系统更新周期：每日(月)次数32(vanilla)
        ...
        // private EntityQuery m_NodeGroup; // vanilla未使用
        ...
        protected override void OnUpdate()
        {
            ...
            // 这一步直接计算 Edge 的最终地价，确保模拟数据的绝对精度。不再依赖 Map 分辨率。
            var edgeJob = new EdgeLandValueJob
            {
                ...
                m_Sampler =
                {
                    ...
                    // 辅助尺寸 (用于采样其他系统的Map)
                    m_MapSize = CellMapSystemRe.kMapSize,
                    m_AttractionMapSize = CellMapSystemRe.LandValueSystemkTextureSize,
                },

                // 传入配置值(可改为ModSetting输入)
            ...
            // --- Phase 2: Map Rasterization (UI 可视化) ---
            // 这一步将计算好的 Edge 地价“画”到 Map 上。完全为了 UI 服务。
            ...
            var gridJob = new LandValueToGridJob
            {
                ...
                // Read Component (读取 Phase 1 刚算好的值)
                ...
                // m_CellSize = CellMapSystemRe.kMapSize / kTextureSize, // 计算格大小已移除,改为物理距离
            ...
            // 必须等待 Phase 1 完成，因为 Phase 2 要读 Phase 1 的结果
            ...
            // --- 资源管理 ---
            AddWriter(Dependency); // 注册系统对 Map 的写操作
        ...
        public struct EnvironmentSampler
        {
            ...
            {
                // 1. 采样基础数据
                float gPol = CellMapSystemRe.GroundPollutionSystemGetPollution(pos, m_GroundPollutionMap).m_Pollution;
                float aPol = CellMapSystemRe.AirPollutionSystemGetPollution(pos, m_AirPollutionMap).m_Pollution;
                float nPol = CellMapSystemRe.NoisePollutionSystemGetPollution(pos, m_NoisePollutionMap).m_Pollution;
                float waterPol = WaterUtils.SamplePolluted(ref m_WaterSurfaceData, pos);

                float availX = CellMapSystemRe.AvailabilityInfoToGridSystemGetAvailabilityInfo(pos, m_AvailabilityInfoMap).m_AvailabilityInfo.x;
            ...
            {
                int2 cell = CellMapSystemRe.GetCell(p, m_MapSize, m_AttractionMapSize);
        ...
        private struct LandValueToGridJob : IJobParallelFor
        {
            ...
            {
                // 获取当前像素点的世界坐标中心
                float3 cellCenter = CellMapSystemRe.GetCellCenter(index, m_TextureSize);
                ...
                NetIterator iterator = new NetIterator
                {
                    ...
                    m_InvRadiusSq = 1.0f / radiusSq, // 预计算半径倒数，避免除法
                    m_Baseline = baseline,
                ...
                {
                    // 道路的平均地价值 (Average Road Value)
                    // 注意：在EdgeJob里已经把环境值加进去了，所以这里直接用道路值即可
                    // 此处如果有建筑，对应地价（含服务+环境）：
                    float roadValue = iterator.m_WeightedSum / iterator.m_TotalWeight;

                    // [优化] 处理边缘过渡问题 (Blending)
                    // 纯粹的IDW在权重低时可能会导致数值跳变，这里做一个简单的混合
                    // 如果只有一点点权重（很远），应该让它逐渐趋向于 backgroundValue
                    // 既然 EdgeJob 里的环境计算和上面的 A步骤环境计算使用的是同一套参数，
                    // 理论上 RoadValue 在去除服务加成后，应该非常接近 BackgroundValue。
                    // 因此，直接使用 RoadValue 也是合理的，或者取二者最大值。
                ...
                // 仅当差异较大时Lerp，节省性能，参数可调 (0.4f 是原版数值)
            ...
            {
                // 输出累加器
                public float m_TotalWeight; // 总权重
                public float m_WeightedSum; // 累加值（Value * Weight）
                                            // 输入参数
                public float3 m_CellCenter; // 像素中心
                                            // public float m_Radius;      // 搜索半径
                public Bounds3 m_Bounds;    // 查询包围盒
                public float m_RadiusSq;
                public float m_InvRadiusSq; // [Opt] 预计算 1/r^2
                ...
                public void Iterate(QuadTreeBoundsXZ bounds, Entity entity)
                {
                    ...
                    if (distSqReal < m_RadiusSq)
                    {
                        ...
                        float t = 1.0f - x; // 线性衰减部分
                        float weight = t * t; // 平方使衰减更平滑(可再叠加幂次使之更平滑)
