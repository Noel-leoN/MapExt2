    ...
    public struct CalculateAveragePollutionJob : IJobChunk
    {
        ...
        public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
        {
            ...
            for (int i = 0; i < chunk.Count; i++)
            {
                ...
                int2 airPollutionBonuses = CellMapSystemRe.GetAirPollutionBonuses(property, ref this.m_TransformFromEntity, this.m_AirPollutionMap, cityModifiers, in this.m_HappinessParameters);
                num3 += airPollutionBonuses.x + airPollutionBonuses.y;
                num6++;
                int2 groundPollutionBonuses = CellMapSystemRe.GetGroundPollutionBonuses(property, ref this.m_TransformFromEntity, this.m_GroundPollutionMap, cityModifiers, in this.m_HappinessParameters);
                num += groundPollutionBonuses.x + groundPollutionBonuses.y;
                num4++;
                int2 noiseBonuses = CellMapSystemRe.GetNoiseBonuses(property, ref this.m_TransformFromEntity, this.m_NoisePollutionMap, in this.m_HappinessParameters);
