...
namespace MapExtPDX.MapExt.ReBurstSystemModeB
{
    ...
    public struct PolluteGroundWaterJob : IJob
    {
        ...
        {
            // ÷ÿ∂®œÚ
            int kTextureSize = CellMapSystemRe.GroundWaterSystemkTextureSize;

            for (int i = 0; i < m_GroundWaterMap.Length; i++)
            {
                GroundWater value = m_GroundWaterMap[i];
                GroundPollution pollution = CellMapSystemRe.GroundPollutionSystemGetPollution(CellMapSystemRe.GetCellCenter(i, kTextureSize), m_PollutionMap);
