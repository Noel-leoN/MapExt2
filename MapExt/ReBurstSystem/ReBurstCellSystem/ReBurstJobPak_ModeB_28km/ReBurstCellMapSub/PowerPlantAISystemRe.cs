    ...
    public struct PowerPlantTickJob : IJobChunk
    {
        ...
        public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
        {
            ...
            for (int i = 0; i < chunk.Count; i++)
            {
                ...
                {
                    // mod
                    Wind wind = CellMapSystemRe.WindSystemGetWind(nativeArray6[i].m_Position, this.m_WindMap);
        ...
        {
            // mod
            float num = (float)CellMapSystemRe.GroundWaterSystemGetGroundWater(position, groundWaterMap).m_Amount / (float)groundWaterData.m_MaximumGroundWater;
