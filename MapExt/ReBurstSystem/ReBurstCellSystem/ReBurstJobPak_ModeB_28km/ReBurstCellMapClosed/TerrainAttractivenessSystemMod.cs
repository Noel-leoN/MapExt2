...
using MapExtPDX.MapExt.Core;
    ...
    public partial class TerrainAttractivenessSystemMod : BaseCellMapSystem, IJobSerializable
    {
        ...
        public static readonly int kTextureSize = CellMapSystemRe.TerrainAttractivenessSystemkTextureSize; // mod尺寸
        ...
        protected override void OnUpdate()
        {
            ...
            // 必须等待之前的写入完成 (base.Dependency) 以及外部数据的依赖
            ...
            // 3. 调度主计算 Job：根据采集的数据计算吸引力
            // 需要 base.m_ReadDependencies 和 base.m_WriteDependencies 来处理 Map 的读写锁
            ...
            // 主 Job 依赖于 Prepare Job 以及 Map 的读写依赖
        ...
        private struct TerrainAttractivenessPrepareJob : IJobParallelForBatch
        {
            ...
            public void Execute(int startIndex, int count)
            {
                ...
                {
                    float3 cellCenter = CellMapSystemRe.GetCellCenter(i, kTextureSize);
                    ...
                    float forestAmbience = CellMapSystemRe.ZoneAmbienceSystemGetZoneAmbience(GroupAmbienceType.Forest, cellCenter, m_ZoneAmbienceData.m_Buffer, 1f);
        ...
        private struct TerrainAttractivenessJob : IJobParallelForBatch
        {
            ...
            public void Execute(int startIndex, int count)
            {
                for (int i = startIndex; i < startIndex + count; i++)
                {
                    float3 currentCellCenter = CellMapSystemRe.GetCellCenter(i, kTextureSize);
                    ...
                    for (int yOffset = -searchRadius; yOffset <= searchRadius; yOffset++)
                    {
                        for (int xOffset = -searchRadius; xOffset <= searchRadius; xOffset++)
                        {
                            ...
                            float distance = math.distance(CellMapSystemRe.GetCellCenter(neighborIndex, kTextureSize), currentCellCenter);

                            // 计算森林吸引力 (neighborData.z 是森林环境值)
                            // 距离越近，森林值越高，吸引力越大
