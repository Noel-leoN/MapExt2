...
namespace MapExtPDX.MapExt.ReBurstSystemModeA
{
    ...
    public struct LandValueTooltipJob : IJob
    {
        ...
        public void Execute()
        {
            ...
            TerrainAttractiveness attractiveness = CellMapSystemRe.TerrainAttractivenessSystemGetAttractiveness(m_RaycastPosition, m_AttractiveMap);
            ...
            m_GroundPollutionResult.value = CellMapSystemRe.GroundPollutionSystemGetPollution(m_RaycastPosition, m_GroundPollutionMap).m_Pollution;
            m_AirPollutionResult.value = CellMapSystemRe.AirPollutionSystemGetPollution(m_RaycastPosition, m_AirPollutionMap).m_Pollution;
            m_NoisePollutionResult.value = CellMapSystemRe.NoisePollutionSystemGetPollution(m_RaycastPosition, m_NoisePollutionMap).m_Pollution;
        }

        public static int GetCellIndex(float3 pos)
        {

            int num = CellMapSystemRe.kMapSize / CellMapSystemRe.LandValueSystemkTextureSize;
            return Mathf.FloorToInt((CellMapSystemRe.kMapSize / 2 + pos.x) / num) + Mathf.FloorToInt((CellMapSystemRe.kMapSize / 2 + pos.z) / num) * CellMapSystemRe.LandValueSystemkTextureSize;
        }
    }
