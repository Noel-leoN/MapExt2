...
namespace MapExtPDX.MapExt.ReBurstSystemModeA
{
        ...
        public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
        {
            ...
            for (int i = 0; i < chunk.Count; i++)
            {
                ...
                if (componentData.m_Types != AllowedWaterTypes.None)
                {
                    ...
                    {
                        // mod CellMapSystemRe.GroundWaterSystemGetGroundWater
                        int num2 = Mathf.RoundToInt(math.clamp((float)CellMapSystemRe.GroundWaterSystemGetGroundWater(nativeArray3[i].m_Position, this.m_GroundWaterMap).m_Max / this.m_Parameters.m_GroundwaterPumpEffectiveAmount, 0f, 1f) * (float)componentData.m_Capacity);
                        num += num2;
                    }
    ...
    public struct GroundWaterPumpJob : IJobChunk
    {
        ...
        public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
        {
            ...
            for (int i = 0; i < chunk.Count; i++)
            {
                ...
                {
                    // mod CellMapSystemRe.GroundWaterSystemGetGroundWater 
                    int num = Mathf.CeilToInt(math.clamp((float)CellMapSystemRe.GroundWaterSystemGetGroundWater(nativeArray3[i].m_Position, this.m_GroundWaterMap).m_Max / this.m_Parameters.m_GroundwaterPumpEffectiveAmount, 0f, 1f) * (float)componentData.m_Capacity);
    ...
    public struct GroundWaterReservoirJob : IJob
    {
        ...
        public void Execute()
        {
            ...
            {
                int index = item.x + item.y * CellMapSystemRe.GroundWaterSystemkTextureSize;
        ...
        {
            if (CellMapSystemRe.GroundWaterSystemIsValidCell(cell) && processedCells.Add(cell))
            {
