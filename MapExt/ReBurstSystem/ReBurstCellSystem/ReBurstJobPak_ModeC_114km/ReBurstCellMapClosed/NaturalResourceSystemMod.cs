...
using MapExtPDX.MapExt.Core;
    ...
    public partial class NaturalResourceSystemMod : BaseCellMapSystem, IJobSerializable, IPostDeserialize
    {
        ...
        public static readonly int kTextureSize = CellMapSystemRe.NaturalResourceSystemkTextureSize; // mod尺寸
        public int2 TextureSize => new int2(kTextureSize, kTextureSize);

        // 地图尺寸
        // public const int MapSize = CellMapSystemRe.kMapSize;

        // 系统更新周期：每日(月)32次
        public const int UPDATES_PER_DAY = 32;
        // 在模拟阶段减少更新频率，其他阶段每帧更新
        public override int GetUpdateInterval(SystemUpdatePhase phase) => (phase == SystemUpdatePhase.GameSimulation) ? 262144 / UPDATES_PER_DAY : 1;
        ...
        protected override void OnUpdate()
        {
            ...
            RegenerateNaturalResourcesJob regenerateJob = new RegenerateNaturalResourcesJob
            {
                ...
                m_MapOffset = -0.5f * CellMapSystemRe.kMapSize,
                m_CellSize = (float)CellMapSystemRe.kMapSize / kTextureSize,
                ...
                m_CellData = base.GetData(false, out var depCellData),
            ...
            // 5. 调度 Job 链
            JobHandle deps = JobUtils.CombineDependencies(depGroundPollution, depNoisePollution, depAreaTree, depCellData, waterDep);
            ...
            JobHandle finalHandle = updateAreaJob.Schedule(updateList, 1, JobHandle.CombineDependencies(collectHandle, depObjTree, depGroundWater));
        ...
        private struct RegenerateNaturalResourcesJob : IJobParallelFor
        {
            ...
            public TargetCellMapData m_CellData;
            ...
            public void Execute(int index)
            {
                ...
                for (int x = 0; x < m_CellData.m_TextureSize.x; x++)
                {
                    ...
                    // --- 1. 肥力计算 (Fertility) ---
                    // 肥力会随时间恢复，但会被地面污染减少
                    int pollutionDamage = MathUtils.RoundToIntRandom(ref random, (float)groundPollution.m_Pollution * m_PollutionRate);
                    int recoveredFertility = cell.m_Fertility.m_Used - m_FertilityRegenerationRate + pollutionDamage;

                    // 限制范围：不能小于0，不能超过基础肥力值
                    cell.m_Fertility.m_Used = (ushort)math.min(cell.m_Fertility.m_Base, math.max(0, recoveredFertility));
                    ...
                    {
                        cell.m_Fish.m_Used = (ushort)math.max(pollutionLimit, cell.m_Fish.m_Used - m_FishRegenerationRate);
                    }
        ...
        private struct AreaIterator : INativeQuadTreeIterator<AreaSearchItem, QuadTreeBoundsXZ>, IUnsafeQuadTreeIterator<AreaSearchItem, QuadTreeBoundsXZ>
        {
            ...
            {
                if (MathUtils.Intersect(bounds.m_Bounds.xz, this.m_Bounds) && this.m_MapFeatureElements.HasBuffer(item.m_Area))
                {
                    // 进一步精确检测：检查三角形与资源单元格是否相交
                    Triangle2 triangle = AreaUtils.GetTriangle2(this.m_Nodes[item.m_Area], this.m_Triangles[item.m_Area][item.m_Triangle]);
