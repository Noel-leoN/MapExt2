            ...
            if (methodBody != null)
            {
                foreach (var localVar in methodBody.LocalVariables)
                {
                    if (localVar.LocalType != null &&
                        context.JobReplacements.TryGetValue(localVar.LocalType, out Type newType))
                    {
                        localRedirects[localVar.LocalIndex] = il.DeclareLocal(newType);
#if DEBUG
                        Info($"    🔎 发现待替换变量 [{localVar.LocalIndex}] | {localVar.LocalType.Name} -> {newType.Name}");
#endif
                    }
                }
            }
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                if (instruction.opcode == OpCodes.Stfld && instruction.operand is FieldInfo oldFieldInfo)
                {
                    if (context.FieldReplacements.TryGetValue(oldFieldInfo, out FieldInfo newFieldInfo))
                    {
                        ...
                        {
                            // 类型不一致，进行安全位投影转换 (规避 IL2CPP Strict Aliasing UB)
#if DEBUG
                            Info(
                                $"    🧬 [Stfld] 安全位转换 {oldFieldInfo.Name} ({oldFieldInfo.FieldType.Name}) -> {newFieldInfo.Name} ({newFieldInfo.FieldType.Name})");
#endif
                            var bitcastMethod = AccessTools.Method(typeof(GenericJobReplacePatch), nameof(Bitcast))
                                .MakeGenericMethod(oldFieldInfo.FieldType, newFieldInfo.FieldType);
                    ...
                    instruction.operand is FieldInfo fieldOp)
                {
                    if (context.FieldReplacements.TryGetValue(fieldOp, out FieldInfo newField))
                    {
                        if (fieldOp.FieldType != newField.FieldType)
                        {
                            ...
                            {
                                Warn($"    ⚠️ [Ldflda] 遇到类型不一致的地址加载，这可能导致 IL 指针解引用时的类型安全问题: {fieldOp.Name}");
                            }
                            else
                            {
                                // 反向位投影 (Reverse Bitcast) - Ldfld 专用
#if DEBUG
                                Info(
                                    $"    🧬 [Ldfld] 安全反向位转换 {newField.Name} ({newField.FieldType.Name}) -> {fieldOp.Name} ({fieldOp.FieldType.Name})");
#endif
                                var bitcastMethod = AccessTools.Method(typeof(GenericJobReplacePatch), nameof(Bitcast))
                                    .MakeGenericMethod(newField.FieldType, fieldOp.FieldType);

                                var newInstLdfld = new CodeInstruction(instruction.opcode, newField)
                                {
                                    labels = instruction.labels
                                };
                                yield return newInstLdfld;
                                yield return new CodeInstruction(OpCodes.Call, bitcastMethod);
                                continue;
                            }
                        }
                ...
                // 5. 泛型与类型引用替换
                {
                    ...
                    if (instruction.operand is Type opType &&
                        context.JobReplacements.TryGetValue(opType, out Type repType))
                    {
                             ...
                             instruction.operand is MethodInfo method && method.IsGenericMethod)
                    {
                        ...
                        var newArgs = args.Select(t =>
                            context.JobReplacements.ContainsKey(t) ? context.JobReplacements[t] : t).ToArray();
