    ...
    public struct ValidationJob : IJobParallelForDefer
    {
        ...
        public void Execute(int index)
        {
            ...
            if (archetypeChunk.Has(ref this.m_ChunkType.m_Object))
            {
                ...
                NativeArray<global::Game.Objects.Transform> nativeArray4 = archetypeChunk.GetNativeArray(ref this.m_ChunkType.m_Transform);
                NativeArray<Attached> nativeArray5 = archetypeChunk.GetNativeArray(ref this.m_ChunkType.m_Attached);
                NativeArray<global::Game.Objects.NetObject> nativeArray6 = archetypeChunk.GetNativeArray(ref this.m_ChunkType.m_NetObject);
                ...
                for (int i = 0; i < nativeArray.Length; i++)
                {
                    ...
                    if ((temp.m_Flags & tempFlags) == 0)
                    {
                        ...
                        global::Game.Objects.Transform transform = nativeArray4[i];
                ...
                for (int k = 0; k < nativeArray6.Length; k++)
                {
                    if ((nativeArray2[k].m_Flags & (TempFlags.Delete | TempFlags.Select | TempFlags.Duplicate)) == 0)
                    {
                        ...
                        global::Game.Objects.NetObject netObject = nativeArray6[k];
                        global::Game.Objects.Transform transform3 = nativeArray4[k];
                        ...
                        global::Game.Objects.ValidationHelpers.ValidateNetObject(entity3, owner2, netObject, transform3, prefabRef3, attached2, this.m_EntityData, this.m_ErrorQueue);
                    }
                }
                if (archetypeChunk.Has(ref this.m_ChunkType.m_TransportStop))
                {
                    for (int l = 0; l < nativeArray.Length; l++)
                    {
                        ...
                        if ((temp2.m_Flags & (TempFlags.Delete | TempFlags.Select | TempFlags.Duplicate)) == 0)
                        {
                            ...
                            global::Game.Objects.Transform transform4 = nativeArray4[l];
                            ...
                            global::Game.Routes.ValidationHelpers.ValidateStop(this.m_EditorMode, entity4, temp2, owner3, transform4, prefabRef4, attached3, this.m_EntityData, this.m_ErrorQueue);
                        }
                    }
                }
                if (flag)
                {
                    for (int m = 0; m < nativeArray.Length; m++)
                    {
                        if ((nativeArray2[m].m_Flags & (TempFlags.Delete | TempFlags.Select | TempFlags.Duplicate)) == 0)
                        {
                            ...
                            global::Game.Objects.Transform transform5 = nativeArray4[m];
                            global::Game.Objects.ValidationHelpers.ValidateOutsideConnection(entity5, transform5, this.m_TerrainHeightData, this.m_ErrorQueue);
                        }
                    }
                }
            }
            if (archetypeChunk.Has(ref this.m_ChunkType.m_ServiceUpgrade))
            {
                ...
                for (int n = 0; n < nativeArray9.Length; n++)
                {
                    ...
                    global::Game.Buildings.ValidationHelpers.ValidateUpgrade(entity6, owner4, prefabRef5, this.m_EntityData, this.m_ErrorQueue);
                }
            }
            if (archetypeChunk.Has(ref this.m_ChunkType.m_Edge))
            {
                ...
                NativeArray<global::Game.Net.Edge> nativeArray15 = archetypeChunk.GetNativeArray(ref this.m_ChunkType.m_Edge);
                ...
                for (int num = 0; num < nativeArray16.Length; num++)
                {
                    ...
                    if ((temp3.m_Flags & tempFlags) == 0)
                    {
                        ...
                        global::Game.Net.Edge edge = nativeArray15[num];
            ...
            if (archetypeChunk.Has(ref this.m_ChunkType.m_Lane))
            {
                ...
                NativeArray<global::Game.Net.TrackLane> nativeArray26 = archetypeChunk.GetNativeArray(ref this.m_ChunkType.m_TrackLane);
                ...
                for (int num2 = 0; num2 < nativeArray26.Length; num2++)
                {
                    if ((nativeArray23[num2].m_Flags & (TempFlags.Delete | TempFlags.Select | TempFlags.Duplicate)) == 0)
                    {
                        ...
                        global::Game.Net.TrackLane trackLane = nativeArray26[num2];
                        ...
                        global::Game.Net.ValidationHelpers.ValidateLane(entity8, owner6, lane, trackLane, curve, edgeLane, prefabRef7, this.m_EntityData, this.m_ErrorQueue);
                    }
                }
            }
            if (archetypeChunk.Has(ref this.m_ChunkType.m_Area))
            {
                ...
                BufferAccessor<global::Game.Areas.Node> bufferAccessor = archetypeChunk.GetBufferAccessor(ref this.m_ChunkType.m_AreaNode);
                ...
                for (int num3 = 0; num3 < nativeArray30.Length; num3++)
                {
                    ...
                    if ((temp4.m_Flags & tempFlags) == 0)
                    {
                        ...
                        DynamicBuffer<global::Game.Areas.Node> nodes = bufferAccessor[num3];
                        ...
                        global::Game.Areas.ValidationHelpers.ValidateArea(this.m_EditorMode, entity9, temp4, owner7, area, geometry, storage, nodes, prefabRef8, this.m_EntityData, this.m_ObjectSearchTree, this.m_NetSearchTree, this.m_AreaSearchTree, this.m_WaterSurfaceData, this.m_TerrainHeightData, this.m_ErrorQueue);
                    }
                }
            }
            if (archetypeChunk.Has(ref this.m_ChunkType.m_RouteSegment))
            {
                ...
                for (int num4 = 0; num4 < nativeArray37.Length; num4++)
                {
                    ...
                    {
                        global::Game.Routes.ValidationHelpers.ValidateRoute(nativeArray37[num4], prefabRef: nativeArray39[num4], waypoints: bufferAccessor2[num4], segments: bufferAccessor3[num4], temp: temp5, data: this.m_EntityData, errorQueue: this.m_ErrorQueue);
                    }
                }
            }
            if (!this.m_EditorMode && archetypeChunk.Has(ref this.m_ChunkType.m_Brush))
            {
                ...
                for (int num5 = 0; num5 < nativeArray40.Length; num5++)
                {
                    ...
                    if (this.m_EntityData.m_TerraformingData.HasComponent(brush.m_Tool))
                    {
                        ...
                        global::Game.Areas.ValidationHelpers.BrushAreaIterator iterator = new global::Game.Areas.ValidationHelpers.BrushAreaIterator
                        {
                        ...
                        global::Game.Objects.ValidationHelpers.ValidateWorldBounds(entity10, default(Owner), bounds, this.m_EntityData, this.m_TerrainHeightData, this.m_ErrorQueue);
                    }
                }
            }
            ...
            NativeArray<global::Game.Objects.Transform> nativeArray44 = archetypeChunk.GetNativeArray(ref this.m_ChunkType.m_Transform);
            NativeArray<global::Game.Simulation.WaterSourceData> nativeArray45 = archetypeChunk.GetNativeArray(ref this.m_ChunkType.m_WaterSourceData);
            for (int num6 = 0; num6 < nativeArray42.Length; num6++)
            {
                ...
                if ((temp6.m_Flags & (TempFlags.Delete | TempFlags.Select | TempFlags.Duplicate)) == 0 || (temp6.m_Flags & TempFlags.Dragging) != 0)
                {
                    ...
                    global::Game.Objects.Transform transform6 = nativeArray44[num6];
                    global::Game.Simulation.WaterSourceData waterSourceData = nativeArray45[num6];
                    global::Game.Objects.ValidationHelpers.ValidateWaterSource(entity11, transform6, waterSourceData, this.m_TerrainHeightData, this.m_worldBounds, this.m_ErrorQueue);
                }
            }
        }

        public static void ValidateBuilding(Entity entity, Building building, Game.Objects.Transform transform, PrefabRef prefabRef, ValidationSystem.EntityData data, NativeArray<GroundWater> groundWaterMap, NativeQueue<ErrorData>.ParallelWriter errorQueue)
        {
            if (building.m_RoadEdge == Entity.Null)
            {
                BuildingData buildingData = data.m_PrefabBuilding[prefabRef.m_Prefab];
                if ((buildingData.m_Flags & global::Game.Prefabs.BuildingFlags.RequireRoad) != 0)
                {
                    float3 position = BuildingUtils.CalculateFrontPosition(transform, buildingData.m_LotSize.y);
                    bool num = (buildingData.m_Flags & (global::Game.Prefabs.BuildingFlags.CanBeOnRoad | global::Game.Prefabs.BuildingFlags.CanBeOnRoadArea)) != 0;
                    bool flag = (buildingData.m_Flags & global::Game.Prefabs.BuildingFlags.CanBeRoadSide) != 0;
                    if (num && !flag)
                    {
